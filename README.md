...
| [🧬 llm-regressor](https://github.com/siddharthgaur1/llm-regressor) · **[📦 PyPI](https://pypi.org/project/llm-regressor/)** | The pip-installable sibling: model-agnostic (Claude/OpenAI/Ollama/LiteLLM) regression testing you drop into any repo, not a hosted dashboard. Deterministic checks fail closed, judge-scored checks flag a >20% drop vs. baseline, and self-consistency measures embedding variance across repeated runs of the same prompt — exits nonzero on any CRITICAL regression to gate CI | click · rich · pytest · GitHub Actions |
