...
| [🧬 llm-regressor](https://github.com/siddharthgaur1/llm-regressor) · [📦 PyPI](https://pypi.org/project/llm-regressor/) | Pip-installable, model-agnostic (Claude/OpenAI/Ollama/LiteLLM) regression testing for LLM prompt/model changes — drop into any repo's CI | Severity has to fail closed on deterministic checks (wrong format, missing fact) while treating LLM-judge score drops as directional, not absolute — conflating the two would make the gate either too paranoid or too permissive | **14/14 tests passing, 57% coverage** (`pytest --cov`) — measured, including the honest gap: judge/provider code paths that need a live key aren't covered by CI |
