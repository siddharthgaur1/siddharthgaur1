...
| **[llm-regressor](https://github.com/siddharthgaur1/llm-regressor)** | *The library + CI gate.* Model-agnostic regression testing for prompt and model changes; a reusable GitHub Action gates a PR in five lines. 123 tests, 100% statement and branch coverage, ~2s with no API key | — |
| **[llm-regression-detector](https://github.com/siddharthgaur1/llm-regression-detector)** | *The dashboard + alerting.* Same problem from the other end: a standing harness with a golden dataset, per-category scoring, Slack drift alerts | [▶](https://siddharthgaur1-siddharthllm-regression-detector-dashboardapp.streamlit.app/) |
| **[querypilot-v2](https://github.com/siddharthgaur1/querypilot-v2)** | English → SQL with schema-aware RAG (retrieves the 3 relevant table chunks, not a full schema dump). Write-safety is a SQLite `PRAGMA query_only` + authorizer at the DB layer, so a prompt injection that beats every earlier check still can't write | [▶](https://siddharthgaur1-siddharthquerypilot-srcapp-ebww0h.streamlit.app/) |
